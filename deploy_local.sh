...
printf "║    curl -N http://localhost:%s/events%-24s║\n" "${GATEWAY_PORT}" ""
echo "║                                                               ║"
echo "║  Send a message:                                              ║"
echo "║    go run ./cmd/demo send -r ${REGISTRY_URL} \\               ║"
echo "║      -m 'Hello ADCNet!' -b 100 --skip-verification            ║"
echo "║                                                               ║"
echo "║  Monitor rounds (CLI):                                        ║"
echo "║    go run ./cmd/demo monitor -r ${REGISTRY_URL} --follow \\   ║"
echo "║      --skip-verification                                      ║"
echo "║                                                               ║"
echo "║  Check status:                                                ║"
printf "║    curl %s/services | jq%-20s║\n" "${REGISTRY_URL}" ""
echo "║                                                               ║"
printf "║  View logs: tail -f %s/*.log\n" "${TEMP_DIR}"
echo "║                                                               ║"
